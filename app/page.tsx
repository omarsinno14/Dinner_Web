...
export default function Home() {
  ...
  const [hasClickedRevealLocation, setHasClickedRevealLocation] = useState(false);
  ...
  useEffect(() => {
    if (step === "location") {
      setHasClickedRevealLocation(false);
    }
  }, [step]);

  const cardClass = useMemo(
    () =>
      "mx-auto w-full max-w-lg overflow-hidden rounded-3xl border border-black/10 bg-white/80 p-4 shadow-[0_16px_50px_-30px_rgba(0,0,0,0.45)] backdrop-blur sm:p-6 md:p-8",
    []
  );
...
`;
  ...
  return (
    <div
      className="flex min-h-screen w-full flex-col items-center justify-center overflow-x-hidden px-4 py-8 transition-colors duration-700 box-border sm:px-6"
    ...
    >
      <main className="mx-auto w-full max-w-lg">
        <section key={step} className={cardClass}>
          ...
          {step === "time" && <div className="space-y-5 text-center"><p className="text-base sm:text-xl">{SITE_CONFIG.prompts.time}</p><div className="flex flex-wrap justify-center gap-2 pb-2">{SITE_CONFIG.timeSlots.map((slot) => <button key={slot} onClick={() => setFakeTime(slot)} className={`whitespace-nowrap rounded-xl border px-4 py-2 text-sm sm:text-base ${fakeTime === slot ? "border-black bg-black text-white" : "border-black/20"}`}>{slot}</button>)}</div><button disabled={!fakeTime} onClick={() => setTimeSubmitted(true)} className="w-full rounded-xl border border-black/20 px-5 py-2.5 disabled:opacity-40 sm:w-auto">{SITE_CONFIG.buttons.submitTime}</button>{timeSubmitted && <><p className="text-sm sm:text-base">{SITE_CONFIG.prompts.forcedTime}</p><button onClick={() => setStep("location")} className="w-full rounded-xl bg-[#1f1f1f] px-5 py-2.5 text-white sm:w-auto">{SITE_CONFIG.buttons.fineContinue}</button></>}</div>}
          {step === "location" && <div className="space-y-5 text-center"><p className="text-base sm:text-xl">{SITE_CONFIG.prompts.location}</p><div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{[1, 2, 3].map((n) => <button key={n} onClick={() => setLocation(n)} className="min-h-[120px] w-full [perspective:1000px]"><div style={{ transform: `rotateY(${location === n ? 180 : 0}deg)` }} className="relative h-full w-full rounded-2xl border border-black/15 bg-white text-sm transition duration-500 [transform-style:preserve-3d]"><span className="absolute inset-0 grid place-items-center [backface-visibility:hidden]">{n}</span><span className="absolute inset-0 grid place-items-center px-2 text-center [backface-visibility:hidden] [transform:rotateY(180deg)]">{SITE_CONFIG.locationFlipMessages[n as 1 | 2 | 3]}</span></div></button>)}</div><button onClick={() => setHasClickedRevealLocation(true)} className="w-full rounded-xl border border-black/20 px-5 py-2.5 sm:w-auto">{SITE_CONFIG.buttons.revealLocation}</button>{hasClickedRevealLocation && <p className="mt-3 text-center text-sm text-neutral-700">{SITE_CONFIG.prompts.reveal}</p>}{location && <button onClick={() => setStep("terms")} className="w-full rounded-xl bg-[#1f1f1f] px-5 py-2.5 text-white sm:w-auto">{SITE_CONFIG.buttons.confirm}</button>}</div>}
